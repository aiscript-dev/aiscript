...
const PRIMITIVE_PROPS: {
	...
	num: {
		to_str: (target: VNum): VFn =>
			FN_NATIVE(async (_, _opts) => {
				return STR(target.value.toString());
			}),
	},

	str: {
		to_num: (target: VStr): VFn =>
			FN_NATIVE(async (_, _opts) => {
		...
		slice: (target: VStr): VFn =>
			FN_NATIVE(async ([begin, end], _opts) => {
				assertNumber(begin);
				assertNumber(end);
				return STR(substring(target.value, begin.value, end.value));
			}),

		pick: (target: VStr): VFn =>
			FN_NATIVE(async ([i], _opts) => {
				assertNumber(i);
		...
		unicode_codepoint_at: (target: VStr): VFn =>
			FN_NATIVE(([i], _) => {
				assertNumber(i);
	...
	arr: {
		...
		push: (target: VArr): VFn =>
			FN_NATIVE(async ([val], _opts) => {
				expectAny(val);
				target.value.push(val);
				return target;
			}),

		unshift: (target: VArr): VFn =>
			FN_NATIVE(async ([val], _opts) => {
				expectAny(val);
		...
		shift: (target: VArr): VFn =>
			FN_NATIVE(async (_, _opts) => {
				return target.value.shift() ?? NULL;
			}),

		concat: (target: VArr): VFn =>
			FN_NATIVE(async ([x], _opts) => {
				assertArray(x);
				return ARR(target.value.concat(x.value));
			}),

		slice: (target: VArr): VFn =>
			FN_NATIVE(async ([begin, end], _opts) => {
				assertNumber(begin);
				assertNumber(end);
		...
		join: (target: VArr): VFn =>
			FN_NATIVE(async ([joiner], _opts) => {
		...
		reduce: (target: VArr): VFn =>
			FN_NATIVE(async ([fn, initialValue], opts) => {
				assertFunction(fn);
		...
		incl: (target: VArr): VFn =>
			FN_NATIVE(async ([val], _opts) => {
				...
				return getValue(target).includes(val.type === "null" ? null : val.value)
					? TRUE
					: FALSE;
			}),

		reverse: (target: VArr): VFn =>
			FN_NATIVE(async (_, _opts) => {
				target.value.reverse();
				return NULL;
			}),

		copy: (target: VArr): VFn =>
			FN_NATIVE(async (_, _opts) => {
				return ARR([...target.value]);
			}),
		sort: (target: VArr): VFn =>
			FN_NATIVE(async ([comp], opts) => {
