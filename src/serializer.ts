...
export function serializeOne(node: Node | null | undefined): Bin | null {
	...
	switch (node.type) {
		...
		case 'fn': return [types[node.type], node.args.map(arg => arg.name), serialize(node.children), node.ret || null, node.args.map(arg => arg.type || null)];
...
export function deserializeOne(bin: Bin | null): Node | undefined {
	...
	switch (bin[0]) {
		...
		case types.fn: return { type, args: bin[1].map((name: string, i: number) => { return { name, type: bin[4][i] }; }), children: deserialize(bin[2]), ret: bin[3] } as NFn;
