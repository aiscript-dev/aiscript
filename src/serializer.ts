import { Node, NDef, NAssign, NCall, NReturn, NIf, NFor, NForOf, NVar, NNull, NBool, NNum, NStr, NArr, NFn, NObj, NProp, NPropCall, NIndex, NBlock, NTmpl, NNs, NMatch, NMeta, NPropAssign, NIndexAssign, NInc, NDec, NAttr, NBreak, NLoop, NContinue, NInfix, NOperator, NLangVer } from './node';
...
export function serializeOne(node: Node | null | undefined): Bin | null {
	...
	switch (node.type) {
		...
		case 'langVer': return [types[node.type], node.name];
	}
}
...
export function deserializeOne(bin: Bin | null): Node | undefined {
	...
	switch (bin[0]) {
		...
		case types.langVer: return { type, name: bin[1] } as NLangVer;
	}
